  ...
  plugins: {
    'postcss-functions': {
      functions: {
        ...
        bp (minValue, maxValue, mediaQueryChange = 680) {
          // Modo de uso: bp(10, 20, 1024)
          return `clamp(${minValue}px, calc(${minValue}px + (${maxValue} - ${minValue}) * (100vw - ${mediaQueryChange}px)), ${maxValue}px)`
        }
      }
    },
    'postcss-import': {},
